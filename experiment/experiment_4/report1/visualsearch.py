...
DATA_FILENAME = f'data{SUBJECT}.csv'
...
instructions = [
    "画面に表示される図形の中に、他と異なる目標刺激（ターゲット）があるかどうかを判断してください。",
    "ターゲットがある場合は、右手人差し指で 'J' キーを押してください。",
    "ターゲットがない場合は、左手人差し指で 'F' キーを押してください。",
    "できるだけ正確に、かつ素早く反応してください。",
    f"練習試行数は {P_TRIALS} 回、本実験の試行数は {X_TRIALS} 回です。",
    "",
    "準備ができたら、任意のキーを押して開始してください。",
]
instr_stims = []
line_height = 40
...
end_stim1 = visual.TextStim(win, text='All done! Thank you for participating',
                             pos=(0, 40), color=BLACK, height=36, units='pix')
end_stim2 = visual.TextStim(win, text='Press any key',
                             pos=(0, 0), color=BLACK, height=36, units='pix')
end_stim1.draw()
end_stim2.draw()
win.flip()
wait_key()
