...
\begin{document}
  ...
  \section{目的}
  近年，クラウド化の影響のによってネットワークの重要性がより大きくなった．しかし，Amazon Web Service(以下:AWS)やAzure，Google Cloud Platformなどによってネットワーク内部に触れる機会は大きく減少している．それが引き起こす問題として，この実験直前に起こったAWSのシステム障害を起因とする大規模なシステム障害である．これは市場がAWSの寡占状態にあったことも問題の一つであるが，ネットワーク内部に触れるエンジニアの割合が市場の規模に適していないからであると私は考えている．そのため，今実験ではネットワーク内部の仕組みを実際に稼働させながら学び，有事の際に何かしらの対処を行えるようになることが本講義の目的であると考える．

  \section{内容}
  今回構築したネットワークは主にレイヤ3(以下:L3)までの階層で動作する．いくつかの段階からこのネットワークは構築されている．
  \begin{enumerate}[(1). ]
    \item デバイス同士をL2のスイッチ(以下:スイッチ)を用いて，Ethernetで通信を行う．
    \item スイッチのVLAN機能でスイッチを仮想的に2つに分割する．これにより，異なるVLANに接続したデバイス同士は通信が行えない．
    \item その異なるVLANに接続しているデバイス同士の通信を行うためにL3のルータを用いる．ルータに存在するルーティング機能を用いて，2つのセグメントに別れたデバイスに情報を送信する．
    \item 更に上位のルータを用いて，自班のルータのルーティングテーブルに他の班の情報を書き込み，他の班との通信を行う．また，上位ルータはインターネットに接続しているため，yahoo.co.jpには本大学のDNSの機能から接続可能である．
  \end{enumerate}
